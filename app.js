...
const configPaths = [
  path.join(os.homedir(), ".config", "btc-dashboard.env"),
  path.join("/etc", "btc-dashboard", ".env"),
...
if (process.env.BTCEXP_BASIC_AUTH_PASSWORD) {
  ...
  expressApp.use(
    sso(
      process.env.BTCEXP_SSO_TOKEN_FILE,
      process.env.BTCEXP_SSO_LOGIN_REDIRECT_URL
    )
  );
}
...
debugLog(
  `Session config: ${JSON.stringify(
    utils.obfuscateProperties(sessionConfig, ["secret"])
  )}`
);
...
function loadMiningPoolConfigs() {
  ...
  var miningPoolsConfigDir = path.join(
    __dirname,
    "public",
    "txt",
    "mining-pools-configs",
    global.coinConfig.ticker
  );

  fs.readdir(miningPoolsConfigDir, function (err, files) {
    if (err) {
      utils.logError("3ufhwehe", err, {
        configDir: miningPoolsConfigDir,
        desc: "Unable to scan directory",
      });
...
async function getSourcecodeProjectMetadata() {
  var options = {
    url: "https://api.github.com/repos/aliakhgar/btc-dashboard",
    headers: {
      "User-Agent": "request",
    },
  };
    ...
    global.coinConfig.historicalData.forEach(function (item) {
      if (item.chain == chain) {
        if (item.type == "blockheight") {
          ...
          global.specialAddresses[item.address] = {
            type: "fun",
            addressInfo: item,
          };
        }
...
async function onRpcConnectionVerified(getnetworkinfo, getblockchaininfo) {
  ...
  if (match) {
    ...
    debugErrorLog(
      `Unable to parse node version string: ${getnetworkinfo.subversion} - RPC versioning will likely be unreliable. Is your node a version of Bitcoin Core?`
    );
  }
  ...
  if (false) {
    ...
    sock.on("message", function (topic, message) {
      console.log(
        Buffer.from(topic).toString("ascii") +
          " - " +
...
async function loadDifficultyHistory(tipBlockHeight = null) {
  ...
  for (let i = 0; i < heights.length; i++) {
    if (
      global.difficultyHistory[`${heights[i]}`].difficulty >
...
async function assessTxindexAvailability() {
  ...
  try {
    ...
    debugLog(
      `txindex check: getindexinfo=${JSON.stringify(global.getindexinfo)}`
    );
    ...
    debugLog(
      `txindex check: error in rpc getindexinfo; will try again in ${retryTime}ms`
    );
...
expressApp.onStartup = async () => {
  ...
  debugLog(
    `Environment(${expressApp.get("env")}) - Node: ${
      process.version
    }, Platform: ${process.platform}, Versions: ${JSON.stringify(
      process.versions
    )}`
  );
  ...
  if (config.cdn.active && config.cdn.s3Bucket) {
    debugLog(
      `Configuring CDN assets; uploading ${cdnItems.length} assets to S3...`
    );
    ...
    const uploadAssetIfNeeded = async (filepath, contentType, encoding) => {
      try {
        ...
        debugErrorLog(
          `Error uploading asset to S3: ${JSON.stringify(filepath)}`,
          e
        );
      }
    };
...
function connectToRpcServer() {
  ...
  debugLog(
    `RPC Credentials: ${JSON.stringify(
      utils.obfuscateProperties(credentialsForRpcConnect, ["password"]),
      null,
      4
    )}`
  );
  ...
  debugLog(
    `RPC Connection properties: ${JSON.stringify(
      utils.obfuscateProperties(rpcClientProperties, ["password"]),
      null,
      4
    )}`
  );
...
expressApp.continueStartup = function () {
  ...
  if (config.credentials.rpc.authType == "cookie") {
    debugLog(
      `RPC authentication is cookie based; watching for changes to the auth cookie file...`
    );
  ...
  verifyRpcConnection();
  global.verifyRpcConnectionIntervalId = setInterval(
    verifyRpcConnection,
    30000
  );
    ...
    if (config.addressApi == "electrum" || config.addressApi == "electrumx") {
      if (config.electrumServers && config.electrumServers.length > 0) {
        electrumAddressApi
          ...
          .catch(function (err) {
            utils.logError("31207ugf4e0fed", err, {
...
expressApp.use(function (req, res, next) {
  ...
  res.locals.genesisCoinbaseTransactionId =
    coreApi.getGenesisCoinbaseTransactionId();
  ...
  userSettings.localCurrency =
    userSettings.localCurrency || config.displayDefaults.localCurrency;
  userSettings.uiTimezone =
    userSettings.uiTimezone || config.displayDefaults.timezone;
...
const sharedErrorHandler = (req, err) => {
  if (err && err.message && err.message.includes("Not Found")) {
    ...
    debugErrorLog(
      `404 NotFound: path=${path}, ip=${ip}, userAgent=${userAgent} (crawler=${
        crawler != null
      }${crawler ? crawler : ""})`
    );
...
if (
  expressApp.get("env") === "development" ||
  expressApp.get("env") === "local"
) {
