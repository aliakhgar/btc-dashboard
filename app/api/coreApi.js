...
global.miningSummaryLruCache = cacheUtils.lruCache(
  config.slowDeviceMode ? 500 : 4500
);
...
(function () {
  const pruneCaches = () => {
    ...
    statTracker.trackValue(
      "caches.misc.itemCount",
      global.miscLruCache.itemCount
    );

    statTracker.trackValue("caches.block.size", global.blockLruCache.size);
    statTracker.trackValue(
      "caches.block.itemCount",
      global.blockLruCache.itemCount
    );
    ...
    statTracker.trackValue(
      "caches.mining.size",
      global.miningSummaryLruCache.size
    );
    statTracker.trackValue(
      "caches.mining.itemCount",
      global.miningSummaryLruCache.itemCount
    );
...
if (!config.noInmemoryRpcCache) {
  ...
  miscCaches.push(
    cacheUtils.createMemoryLruCache(
      "misc",
      global.miscLruCache,
      onMemoryCacheEvent
    )
  ...
  txCaches.push(
    cacheUtils.createMemoryLruCache("tx", global.txLruCache, onMemoryCacheEvent)
  );
...
if (redisCache.active) {
  ...
  // multiple instances of btc-dashboard (eg mainnet + testnet) to share
  // a single redis instance peacefully
  const rpcHostPort = `${config.credentials.rpc.host}:${config.credentials.rpc.port}`;
  const rpcCredKeyComponent = md5(
    JSON.stringify(config.credentials.rpc)
  ).substring(0, 8);
...
function getGenesisBlockHash() {
  return coins[config.coin].genesisBlockHashesByNetwork[
    global.activeBlockchain
  ];
}

function getGenesisCoinbaseTransactionId() {
  return coins[config.coin].genesisCoinbaseTransactionIdsByNetwork[
    global.activeBlockchain
  ];
}

function tryCacheThenRpcApi(
  cache,
  cacheKey,
  cacheMaxAge,
  rpcApiFunction,
  cacheConditionFunction
) {
...
function getUtxoSetSummary(
  useCoinStatsIndexIfAvailable = true,
...
async function getNextBlockEstimate() {
  ...
  for (let i = 0; i < groupCount; i++) {
    feeRateGroups.push({
      ...
      maxFeeRate:
        minFeeRate + ((i + 1) * (maxFeeRate - minFeeRate)) / groupCount,
      totalWeight: 0,
      txidCount: 0,
      //txids: []
    });
  ...
  const subsidy = coinConfig.blockRewardFunction(
...
async function getDifficultyByBlockHeights(blockHeights) {
  ...
  for (let i = 0; i < blockHeights.length; i++) {
    ...
    if (global.difficultyByBlockheightCache[blockHeightStr]) {
      results[blockHeight] =
        global.difficultyByBlockheightCache[blockHeightStr];
    } else {
...
async function getTxStats(dataPtCount, blockStart, blockEnd) {
  ...
  if (blockStart > blockEnd) {
    throw new Error(
      `Error 37rhw0e7ufdsgf: blockStart (${blockStart}) > blockEnd (${blockEnd})`
    );
  }
  ...
  for (let i = 0; i < dataPtCount; i++) {
    ...
    promises.push(
      (async () => {
        ...
        return await rpcApi.getChainTxStats(
          Math.min(blockCount, blockHeightEnd - 1),
          blockhashEnd
        );
      })()
    );
  ...
  if (
    results.length == 0 ||
    (results[0].name == "RpcError" && results[0].code == -8)
  ) {
  ...
  for (let i = results.length - 1; i >= 0; i--) {
    if (results[i].window_tx_count) {
      summary.txCounts.push({
        x: blockStart + i * blockCount,
        y: results[i].window_tx_count,
      });
      summary.txRates.push({
        x: blockStart + i * blockCount,
        y: results[i].txrate,
      });
      summary.timespans.push({
        x: blockStart + i * blockCount,
        y: results[i].window_interval,
      });
        ...
        if (getpeerinfo.length > 0 && getpeerinfo[0].connection_type) {
          ...
          for (let prop in connectionTypeSummaryMap) {
            if (connectionTypeSummaryMap.hasOwnProperty(prop)) {
              connectionTypeSummary.push([
                prop,
                connectionTypeSummaryMap[prop],
              ]);
            }
          }
  ...
  return new Promise(function (resolve, reject) {
    tryCacheThenRpcApi(
      ...
      .then(function (resultTxids) {
        ...
        for (
          let i = offset;
          i < resultTxids.length && i < offset + limit;
          i++
        ) {
...
function buildBlockAnalysisData(
  blockHeight,
  blockHash,
  txids,
  txIndex,
...
function summarizeBlockAnalysisData(blockHeight, tx, inputs) {
  ...
  if (txSummary.coinbase) {
    let subsidy = global.coinConfig.blockRewardFunction(
      blockHeight,
      global.activeBlockchain
    );
    ...
    for (let i = 0; i < tx.vin.length; i++) {
      ...
      if (inputs) {
        ...
        txSummary.totalInput = txSummary.totalInput.plus(
          new Decimal(inputVout.value)
        );
  ...
  for (let i = 0; i < tx.vout.length; i++) {
    txSummary.totalOutput = txSummary.totalOutput.plus(
      new Decimal(tx.vout[i].value)
    );
    ...
    getRawTransactions(txids, blockhash)
      .then(function (transactions) {
        ...
        for (let i = 0; i < vinIds.length; i++) {
          ...
          promises.push(
            getSummarizedTransactionOutput(vinId.txid, vinId.voutIndex)
          );
        }
          ...
          .then(function (promiseResults) {
            ...
            transactions.forEach(function (tx) {
              ...
              if (tx && tx.vin) {
                for (
                  let i = 0;
                  i < Math.min(maxInputsTracked, tx.vin.length);
                  i++
                ) {
  ...
  return new Promise(function (resolve, reject) {
    getBlockByHash(blockHash)
      .then(function (block) {
        ...
        for (
          let i = txOffset;
          i < Math.min(txOffset + txLimit, block.tx.length);
          i++
        ) {
        ...
        getRawTransactionsWithInputs(txids, config.site.txMaxInput, blockHash)
          .then(function (txsResult) {
            if (txsResult.transactions && txsResult.transactions.length > 0) {
              ...
              block.totalFees =
                utils.getBlockTotalFeesFromCoinbaseTxAndBlockHeight(
                  block.coinbaseTx,
                  block.height
                );
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      for (let height = startBlock; height <= endBlock; height++) {
        ...
        summary.miners[miner].totalFees = summary.miners[miner].totalFees.plus(
          blockSummary.f
        );
        summary.miners[miner].totalSubsidy = summary.miners[
          miner
        ].totalSubsidy.plus(blockSummary.s);
        ...
        summary.overall.totalFees = summary.overall.totalFees.plus(
          blockSummary.f
        );
        summary.overall.totalSubsidy = summary.overall.totalSubsidy.plus(
          blockSummary.s
        );
      ...
      summary.minerNamesSortedByBlockCount.sort(function (a, b) {
        return summary.miners[a].blocks.length > summary.miners[b].blocks.length
          ? -1
          : 1;
      });
...
const mempoolTxFileCache = utils.fileCache(
  config.filesystemCacheDir,
  `mempool-tx-summaries`,
  2
);
...
function buildMempoolSummary(statusId, ageBuckets, sizeBuckets, statusFunc) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const txSummaries = await getMempoolTxSummaries(
        allTxids,
        statusId,
        statusFunc
      );
      ...
      for (let i = 1; i < bucketCount; i++) {
        ...
        if (i > 0 && i < bucketCount - 1) {
          satoshiPerByteBucketLabels.push(
            "[" +
              satoshiPerByteBucketMaxima[i - 1] +
              " - " +
              satoshiPerByteBucketMaxima[i] +
              ")"
          );
        }
      }
      ...
      for (let x = 0; x < txSummaries.length; x++) {
        ...
        let feePerByte =
          new Decimal(txMempoolInfo.f).dividedBy(SATS_PER_BTC).toNumber() /
          weight;
        ...
        for (let i = 0; i < satoshiPerByteBuckets.length; i++) {
          if (satoshiPerByteBuckets[i].maxFeeRate > satoshiPerByte) {
            ...
            satoshiPerByteBuckets[i]["totalFees"] = satoshiPerByteBuckets[i][
              "totalFees"
        ...
        if (!addedToBucket) {
          ...
          satoshiPerByteBuckets[bucketCount - 2]["totalFees"] =
            satoshiPerByteBuckets[bucketCount - 2]["totalFees"].plus(
        ...
        summary["count"]++;
        summary["totalFees"] = summary.totalFees.plus(
          new Decimal(fee).dividedBy(SATS_PER_BTC)
        );
      ...
      summary.satoshiPerByteBucketLabels =
        summary.satoshiPerByteBucketLabels.slice(0, topIndex);
      ...
      if (topIndex < satoshiPerByteBuckets.length) {
        ...
        for (let i = topIndex + 1; i < satoshiPerByteBuckets.length; i++) {
          satoshiPerByteBuckets[topIndex].count +=
            satoshiPerByteBuckets[i].count;
          satoshiPerByteBuckets[topIndex].totalFees = satoshiPerByteBuckets[
            topIndex
          ].totalFees.plus(satoshiPerByteBuckets[i].totalFees);
          satoshiPerByteBuckets[topIndex].totalBytes +=
            satoshiPerByteBuckets[i].totalBytes;
          satoshiPerByteBuckets[topIndex].totalWeight +=
            satoshiPerByteBuckets[i].totalWeight;
          satoshiPerByteBuckets[topIndex].buckets++;
        }

        satoshiPerByteBuckets = satoshiPerByteBuckets.slice(0, topIndex + 1);
        satoshiPerByteBucketMaxima = satoshiPerByteBucketMaxima.slice(
          0,
          topIndex + 1
        );
      }
      ...
      summary["averageFeePerByte"] =
        summary["totalFees"] / summary["totalBytes"];
      ...
      for (let i = 0; i < satoshiPerByteBuckets.length; i++) {
        summary["satoshiPerByteBucketCounts"].push(
          summary["satoshiPerByteBuckets"][i]["count"]
        );
    ...
    try {
      ...
      while (unAddedTxIndexes.length > 0 && blocks.length < 20) {
        ...
        for (let i = 0; i < unAddedTxIndexes.length; i++) {
          ...
          if (
            currentBlock.weight + coinConfig.minTxWeight >
            coinConfig.maxBlockWeight
          ) {
          ...
          if (
            ...
            tx.a.forEach((ancesTxidKey) =>
              currentBlock.txids.add(ancesTxidKey)
            );
        ...
        currentBlock.avgFee = currentBlock.totalFees
          .dividedBy(currentBlock.txCount)
          .toDP(8);
        ...
        lines.forEach(function (line) {
          if (line.startsWith("==")) {
            ...
            sectionName = sectionName
              .substring(0, sectionName.length - 2)
              .trim();
