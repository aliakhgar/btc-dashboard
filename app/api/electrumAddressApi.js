...
function connectToServer(host, port, protocol) {
  return new Promise(function (resolve, reject) {
    ...
    var electrumConfig = { client: "btc-dashboard-v2", version: "1.4" };
    var electrumPersistencePolicy = {
      retryPeriod: 10000,
      maxRetry: 1000,
      callback: null,
    };
    ...
    var onConnect = function (client, versionInfo) {
      debugLog(
        `Connected to Electrum Server @ ${host}:${port} (${JSON.stringify(
          versionInfo
        )})`
      );
    ...
    var onError = function (err) {
      ...
      utils.logError("937gf47dsyde", err, {
        host: host,
        port: port,
        protocol: protocol,
      });
  ...
  return new Promise(function (resolve, reject) {
    ...
    Promise.all(promises.map(utils.reflectPromise)).then(function (results) {
      ...
      if (txidData) {
        ...
        for (
          let i = offset;
          i < Math.min(txidData.length, limit + offset);
          i++
        ) {
          addressDetails.txids.push(txidData[i].tx_hash);
          addressDetails.blockHeightsByTxid[txidData[i].tx_hash] =
            txidData[i].height;
        }
      }
...
function getAddressTxids(addrScripthash) {
  return new Promise(function (resolve, reject) {
    ...
    runOnAllServers(function (electrumClient) {
      ...
      .then(function (results) {
        debugLog(
          `getAddressTxids=${utils.ellipsize(JSON.stringify(results), 200)}`
        );
        ...
        if (
          addrScripthash == coinConfig.genesisCoinbaseOutputAddressScripthash
        ) {
          for (let i = 0; i < results.length; i++) {
            results[i].result.unshift({
              tx_hash:
                coinConfig.genesisCoinbaseTransactionIdsByNetwork[
    ...
    runOnAllServers(function (electrumClient) {
      ...
      .then(function (results) {
        ...
        if (
          ...
          for (let i = 0; i < results.length; i++) {
            var coinbaseBlockReward = coinConfig.blockRewardFunction(
              0,
              global.activeBlockchain
            );
